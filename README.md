...
- **New:** Expanded explicit queries to concurrently fetch mosques (`type: mosque`) and Eidgahs (`keyword: eidgah`) in just **2 optimized API calls** (down from 6), reducing Google Places API costs by **67%**.
...
- **New:** Eid dates now dynamically adjust based on the user's **timezone/country**. A region-aware algorithm maps timezones (e.g. Asia/Dhaka, Asia/Karachi) to local moon sighting authorities for accurate per-country Eid dates.
...
- **Architecture:** Migrated backend from **Supabase** to **Firebase Realtime Database** — free forever, never pauses, always online.
...
- 💾 **Firebase Backend** — Real-time database for reports and votes (free, never pauses).
...
| **Google Places API** | Local mosque/Eidgah POI queries (2 optimized calls) |
| **Firebase Realtime DB** | Persistent database for reports & votes |
